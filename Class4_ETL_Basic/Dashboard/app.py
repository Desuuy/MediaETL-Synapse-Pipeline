...
def main():
    ...
    database_dw = st.sidebar.selectbox("Data Warehouse", ["DW_MediaAnalytics"], index=0)
    database_dm = st.sidebar.selectbox("Data Mart", ["DM_MediaAnalytics"], index=0)
    ...
    # Main content
    tab1, tab2, tab3, tab4 = st.tabs(["📈 Overview", "📅 Daily Summary", "🎯 Contract Analytics", "📊 Content Type Trends"])
    ...
    with tab1:
        ...
        if df_summary is not None and len(df_summary) > 0:
            ...
            with col3:
                duration_hours = (df_summary['TotalDuration'].iloc[0] / 3600) if df_summary['TotalDuration'].iloc[0] else 0
    ...
    with tab2:
        ...
        if df_daily is not None and len(df_daily) > 0:
            ...
            labels = ['Truyền Hình', 'Phim Truyện', 'Giải Trí', 'Thiếu Nhi', 'Thể Thao']
    ...
    with tab3:
        ...
        if df_contracts is not None and len(df_contracts) > 0:
            ...
            fig_bar = px.bar(
                ...
                labels={'TotalDuration_Hours': 'Duration (Hours)', 'ContractID': 'Contract'}
            )
    ...
    with tab4:
        ...
        if df_trends is not None and len(df_trends) > 0:
            ...
            fig_line = px.line(
                ...
                labels={'Duration_Hours': 'Duration (Hours)', 'DateValue': 'Date'}
            )
